...
export const RegisterPage: React.FC = () => {
  ...
  const [captcha, setcaptcha] = useState(12345)
  ...
  function passwordCheck(e) {
    ...
    else if (pass.length >=5  && !(symbolRegex.test(pass))) {
      setpasserror('moderate')
    }
    else if (pass.length >5 && symbolRegex.test(pass)) {
      setpasserror('strong')
    }
  ...
  function checkOTP(e) {
    if(e.target.value !== captcha.toString()){
      setError('OTP donot match')
    }
    else return
    return
  }
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            />
            {passerror == 'weak' && (<p className='text-error-700'>weak</p>)}
            {passerror == 'moderate' && (<p className='text-warning-500'>moderate</p>)}
            {passerror == 'strong' && (<p className='text-success-500'>strong</p>)}
